...
# حالت demo — انتشار فرانت بدون هیچ بک‌اندی

اگر هدف فقط نمایش فروشگاه است (بدون ورود، بدون سفارش واقعی)، فرانت می‌تواند
کاملاً روی دادهٔ نمونهٔ داخلی اجرا شود.

## فعال‌سازی

یک متغیر در Vercel (یا هر پلتفرم دیگر):

```
NEXT_PUBLIC_API_MOCKING=enabled
```

و تمام. `NEXT_PUBLIC_API_URL` لازم نیست.

قبلاً این پرچم شرط `NODE_ENV === "development"` هم داشت، یعنی در بیلد
پروداکشن بی‌اثر بود و سایت منتشرشده ویترین خالی نشان می‌داد. آن شرط حذف شد؛
پرچم حالا opt-in است و بدون آن هیچ تغییری در بیلد عادی ایجاد نمی‌شود.

## چه چیزی کار می‌کند

| مسیر | منبع داده |
|---|---|
| `/` | ۹ محصول، دسته‌ها، کمپین‌ها، برندها |
| `/products` + فیلترها | جست‌وجو، برند، رنگ، سایز، موجودی، مرتب‌سازی |
| `/products/[slug]` | جزئیات + واریانت‌ها |
| `/categories`، `/categories/[slug]` | درخت دسته‌ها |
| جست‌وجو و suggestions | محصولات + دسته‌ها |
| `/campaigns/[slug]` | کمپین‌های فعال |

## چه چیزی هنوز به بک‌اند نیاز دارد

سبد خرید، ورود با OTP، checkout و ثبت سفارش، wishlist، آدرس‌ها، پروفایل،
نظرات، اعلان‌ها و روش‌های ارسال — اینها ذاتاً سمت سرور هستند
(۱۸ endpoint باقی‌مانده). در حالت demo این صفحه‌ها باز می‌شوند ولی عملیاتشان
انجام نمی‌شود.
