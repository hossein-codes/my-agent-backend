/**
 * Canonical error codes returned to clients in `{ code }`.
 *
 * These strings are part of the PUBLIC API contract: the frontend switches on
 * them to render localized, actionable messages. Adding a code is additive and
 * safe; renaming or removing one is a BREAKING change.
 *
 * Naming: `<domain>.<reason>` in snake_case. `common.*` for cross-cutting codes.
 */
